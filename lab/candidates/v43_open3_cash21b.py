...
def agent(observation, configuration=None):
    ...
    if 288 <= step < 696:_R37_HORIZONS[player] = 8  # lab v43_lead8: 4->8; measured 6 > 5 > 4 on this base
...
# --- lab candidate v43_open55: one change vs V43 -----------------------------------------
# Ladder replay 109493975 (Shun): same V43 field on all 719 turns, beat us by 1,928 with a
# different market layer. Its first difference is the opening: BUY_PRODUCT WHEAT 55 +
# SELL WHEAT 55, against V43's BUY 5 / BUY 10 / SELL 60. Only turn 0 changes here, and only
# when V43 proposes its own default opening.
_OPEN55_PARENT = agent
_OPEN55_V43 = [["BUY_PRODUCT", "WHEAT", 5], ["BUY_PRODUCT", "WHEAT", 10], ["SELL", "WHEAT", 60]]
def agent(observation, configuration=None):
    action = _OPEN55_PARENT(observation, configuration)
    try:
        if int(observation.get("step", -1)) == 0 and [list(o) for o in action.get("market") or []] == _OPEN55_V43:
            action["market"] = [["BUY_PRODUCT", "WHEAT", 3], ["SELL", "WHEAT", 3]]
    except Exception:
        pass
    return action
agent = globals().pop("agent")
...
# --- lab candidate v43_open3_carrot = v43_lead8_open3 + carrot tie-break; section copied from v43_carrot: one change vs V43 -----------------------------------------
# Port of v41_rl_1's only learned knob (RL run 2, lab/p2/rl_bias.py): sell fraction for
# CARROT raised by 0.15 of the stock the agent could sell this turn. Same money, but it
# beats an exact clone of itself by ~17 coins, turning mirror ties into wins (validation
# 620000-620019: 37-3 vs raw v41, vs 3-3-34T for the unchanged agent). Never touches
# WHEAT/FERTILIZER, never reorders existing slots: the extra carrot goes in the last slot.
_CARROT_PARENT = agent
_CARROT_DELTA = 0.15
def agent(observation, configuration=None):
    action = _CARROT_PARENT(observation, configuration)
    try:
        market = list(action.get("market") or [])
        held = int((observation.get("private") or {}).get("shed", {}).get("CARROT", 0) or 0)
        if held > 0 and len(market) <= 10:
            idx = next((i for i, o in enumerate(market)
                        if o and o[0] == "SELL" and len(o) > 2 and o[1] == "CARROT"), None)
            bump = int(round(_CARROT_DELTA * held))
            if bump > 0:
                if idx is not None:
                    market[idx] = ["SELL", "CARROT", min(held, int(market[idx][2]) + bump)]
                elif len(market) < 10:
                    market.append(["SELL", "CARROT", bump])
...
# cash21b: arrive at the day-0 seed-buy turn with enough coins for 2 wheat seeds.
#
# Ladder evidence (529 games, iteration 33): win rate is monotone in money held at
# replay step 21, where _r124_seed_budget shaves the WHEAT seed order down to fit the
# cash left after the HIRE reserve:
#     money <20 : n=108  17.6%      40-80: n=119  71.4%
#     money 20-40: n=181  63.0%     80+  : n=121  87.6%
# 20% of our games sit in the <20 bucket and we lose 82% of them, and every game that
# bought fewer than 2 seeds held <=13 coins. Two wheat seeds cost 20.
#
# The turn that sets that balance is the big BUY_PRODUCT WHEAT fill on day 0. Trim it
# by one unit so the seed budget clears 20.
#
# NOTE on the turn index: v43_open3_cash21 (871a5888) tried this and was a measured
# no-op -- 46 of 50 games vs this same parent tied at margin exactly 0. It tested
# day*24+hour == 2 because the fill is recorded at index 2 of a replay, but
# steps[i]["action"] is the action taken from the step i-1 observation, so the agent's
# own clock reads 1 when it emits that order and the market list at 2 is empty.
_CASH21B_PARENT = agent


def agent(observation, configuration=None):
    action = _CASH21B_PARENT(observation, configuration)
    try:
        if int(observation.get("day", -1)) * 24 + int(observation.get("hour", -1)) == 1:
            market = list(action.get("market") or [])
            for i, o in enumerate(market):
                if o and o[0] == "BUY_PRODUCT" and len(o) > 2 and o[1] == "WHEAT" and int(o[2]) > 1:
                    market[i] = ["BUY_PRODUCT", "WHEAT", int(o[2]) - 1]
                    action["market"] = market
                    break
    except Exception:
        pass
