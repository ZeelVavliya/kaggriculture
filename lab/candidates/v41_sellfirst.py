...
# --- lab candidate v41_sellfirst: one change vs v41 --------------------------------------
# Ladder replays (episodes 109163246, 109172393, 109192847) show market variants of v41
# that beat it place SELL orders ahead of HIRE/BUY orders in the same turn. Orders resolve
# slot by slot across both players, so a sale in an earlier slot is priced first.
# This keeps v41's relative order within SELLs and within non-SELLs; only the partition moves.
_SELLFIRST_PARENT = agent
def agent(observation, configuration=None):
    action = _SELLFIRST_PARENT(observation, configuration)
    try:
        market = list(action.get("market") or [])
        sells = [o for o in market if o and o[0] == "SELL"]
        rest = [o for o in market if not (o and o[0] == "SELL")]
        action["market"] = sells + rest
    except Exception:
        pass
    return action
agent = globals().pop("agent")
