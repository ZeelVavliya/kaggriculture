...
# --- lab candidate v41_open20: one change vs v41 -----------------------------------------
# Ladder replay 109204733 (ansheng jhang): identical to v41 on all 719 farmer turns and
# 718/719 market turns; the only difference is the first-turn wheat opening, and it beat
# v41 by 13 coins. Only overrides turn 0, and only when v41 proposes its usual opening.
_OPEN20_PARENT = agent
_OPEN20_V41 = [["BUY_PRODUCT", "WHEAT", 5], ["BUY_PRODUCT", "WHEAT", 10], ["SELL", "WHEAT", 60]]
def agent(observation, configuration=None):
    action = _OPEN20_PARENT(observation, configuration)
    try:
        if int(observation.get("step", -1)) == 0 and [list(o) for o in action.get("market") or []] == _OPEN20_V41:
            action["market"] = [["BUY_PRODUCT", "WHEAT", 20], ["SELL", "WHEAT", 5], ["SELL", "WHEAT", 15]]
    except Exception:
        pass
    return action
agent = globals().pop("agent")
