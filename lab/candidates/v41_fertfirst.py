...
# --- lab candidate v41_fertfirst: one change vs v41 --------------------------------------
# Ladder replays 109163246 (siren2345), 109172393 (Himanshu Kumar), 109192847 (kunihiro):
# v41 variants that beat v41 put SELL FERTILIZER in slot 0 ahead of HIREs (steps 97, 121).
# v41_sellfirst (all sells first) was vetoed 0-50, so only fertilizer sells move; every
# other order keeps v41's exact relative order.
_FERTFIRST_PARENT = agent
def agent(observation, configuration=None):
    action = _FERTFIRST_PARENT(observation, configuration)
    try:
        market = list(action.get("market") or [])
        fert = [o for o in market if o and o[0] == "SELL" and len(o) > 1 and o[1] == "FERTILIZER"]
        if fert:
            action["market"] = fert + [o for o in market if not (o and o[0] == "SELL" and len(o) > 1 and o[1] == "FERTILIZER")]
    except Exception:
        pass
    return action
agent = globals().pop("agent")
