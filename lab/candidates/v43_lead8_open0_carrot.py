...
def agent(observation, configuration=None):
    ...
    try:
        if int(observation.get("step", -1)) == 0 and [list(o) for o in action.get("market") or []] == _OPEN55_V43:
            action["market"] = [["BUY_PRODUCT", "WHEAT", 0], ["SELL", "WHEAT", 0]]
    except Exception:
        pass
    return action
agent = globals().pop("agent")
...
# --- lab candidate v43_lead8_open0_carrot = v43_lead8_open0 + carrot tie-break; section copied from v43_carrot: one change vs V43 -----------------------------------------
