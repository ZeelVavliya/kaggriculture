...
# cash21: arrive at step 21 with enough coins for 2 wheat seeds.
# Ladder evidence (529 games): the step-2 fill leaves us either ~6 or ~26 coins at
# step 21; the gap opens at step 2 and never changes again. Two wheat seeds cost 20,
# so the low side gets shaved to 0-2 seeds by _r124_seed_budget and wins 17.6% vs 72.4%.
# Trim one unit off the step-2 wheat buy (~28 coins) to stay on the high side.
_CASH21_PARENT = agent

def agent(observation, configuration=None):
    action = _CASH21_PARENT(observation, configuration)
    try:
        if int(observation.get("day", -1)) * 24 + int(observation.get("hour", -1)) == 2:
            market = list(action.get("market") or [])
