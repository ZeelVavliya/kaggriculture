...
# --- Survival guard overlay -------------------------------------------------
# Finding (lab.audit on tourney-0a651d40, 160/160 c94 games, both seats, every
# seed -- deterministic since the field tape does not branch on the opponent):
# a COW on the PASTURE at (7,4), placed day 8 and fed normally through day 19,
# stops being fed on day 20 (consecutive_unfed goes 0->1 at the day 20/21
# boundary) while the recorded tape spends day 20-21 building and feeding a
# *second* pasture+cow one tile away at (7,3) (DIG/BUILD_PASTURE/PLACE/FEED,
# steps 518-521,526). Day 21 is also missed and it escapes at the day 21/22
# boundary (step 528) in every game. It reads as a genuine gap in the source
# trajectory rather than a deliberate trade-off: at step 498 (day 20, hour 18)
# a hand stands exactly on (7,4) carrying 1 wheat with a scheduled op of
# 'SOUTH' -- it walks across the tile without feeding it. Remaining season
# after day 20 is ~9 days; even a depressed milk price (market floor $1,
# typically far above that mid-game) times that many scheduled milkings clears
# the ~1 wheat/day feed cost many times over, so recovering the cow is net
# positive.
#
# Once day 20 is missed too, no rescue is possible within the safety rules
# below: lab/candidates checked every hand for the rest of day 21 and found no
# idle, wheat-carrying hand within walking distance of (7,4) (nearest is 6-7
# tiles away with only 1-2 turns left in the day) -- the only real fix is to
# not miss day 20 in the first place.
#
# Mechanism, modeled directly on C92's weed-repair interrupt-and-resume
# (_weed_repair_productive_route above): late in the day, if a hand is
# standing exactly on an unfed animal structure it is carrying WHEAT for and
# its *scheduled op this turn is PASS or a bare move* (never
# PLANT/WATER/HARVEST/FEED/CARE/COLLECT_FERTILIZER/PLACE/BUILD_*/DIG or a
# market order), swap in FEED and push the displaced op onto a one-item
# pending queue so it fires the very next turn -- the unit never changes tile
# (FEED does not move it), so this is a same-hour, same-position substitution:
# the rest of the day's route is delayed by exactly one turn and then resumes
# untouched. The farmer is never diverted (its position persists across days,
# unlike hands, so any drift there would permanently desync every following
# day's relative moves).
_sg_host_agent = agent
_SG_LATE_HOUR = 18  # last 6 hours of the day
_SG_STRUCTS = ("PASTURE", "COOP")
_SG_SAFE_OPS = {"PASS", "NORTH", "SOUTH", "EAST", "WEST"}
_sg_pending = {}
_sg_last_step = -1
_sg_day = -1


def _sg_reset(step, day):
    global _sg_pending, _sg_last_step, _sg_day
    if step == 0 or step <= _sg_last_step or day != _sg_day:
        _sg_pending = {}
    _sg_last_step = step
    _sg_day = day


def _sg_tile_at(tiles, position):
    try:
        x, y = int(position[0]), int(position[1])
        return tiles[y][x]
    except (IndexError, TypeError, ValueError):
...
def _sg_guard(obs, action):
    ...
    for actor, position in enumerate(positions):
        if actor == 0:
            continue  # farmer persists across days; never divert it
        scheduled = ops[actor] if isinstance(ops[actor], list) and ops[actor] else ["PASS"]
        pending = _sg_pending.get(actor)
        if pending:
            ops[actor] = pending.pop(0)
            if scheduled[0] != "PASS":
                pending.append(scheduled)
            if pending:
                _sg_pending[actor] = pending
            else:
                _sg_pending.pop(actor, None)
            continue

        if hour < _SG_LATE_HOUR or scheduled[0] not in _SG_SAFE_OPS:
            continue
        tile = _sg_tile_at(tiles, position)
        if not (
            isinstance(tile, dict)
            and tile.get("kind") in _SG_STRUCTS
            and tile.get("animal")
            and not tile.get("fed_today")
        ):
            continue
        wheat = int((inventories[actor] or {}).get("WHEAT", 0) or 0)
        if wheat <= 0:
            continue
        ops[actor] = ["FEED", "WHEAT"]
        _sg_pending[actor] = [scheduled]
...
def agent(obs, config=None):
    action = _sg_host_agent(obs, config)
    try:
        return _sg_guard(obs, action)
    except Exception:
        return action


# Fresh final binding so both callable- and file-path execution pick this
# policy (see the identical note above c94_submission_agent).
def c94sg_submission_agent(obs, config=None):
    return agent(obs, config)
