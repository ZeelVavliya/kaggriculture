...
# --- lab candidate v43_open3_notopup: one change ------------------------------------------
# Ladder analysis over 183 v43_lead8 games: in 26 of its 79 mirror losses we place
# ['BUY_PRODUCT','WHEAT',1 or 2] at step 171 and again at step 195 while the opponent places
# no wheat order at all -- always that direction, never reversed (34.2% of mirror losses).
# This drops those two top-ups only; every other order, including all other wheat buys, is
# left exactly as the parent emitted it.
_NOTOPUP_PARENT = agent
_NOTOPUP_STEPS = (171, 195)
def agent(observation, configuration=None):
    action = _NOTOPUP_PARENT(observation, configuration)
    try:
        if int(observation.get("step", -1)) in _NOTOPUP_STEPS:
            market = action.get("market") or []
            kept = [o for o in market
                    if not (o and o[0] == "BUY_PRODUCT" and len(o) > 2 and o[1] == "WHEAT" and int(o[2]) <= 2)]
