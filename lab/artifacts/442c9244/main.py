...
def agent(observation, configuration=None):
    ...
    try:
        if int(observation.get("step", -1)) == 0 and [list(o) for o in action.get("market") or []] == _OPEN55_V43:
            action["market"] = [["BUY_PRODUCT", "WHEAT", 43], ["SELL", "WHEAT", 43]]
    except Exception:
        pass
    return action
agent = globals().pop("agent")
...
# --- lab candidate v43_lead8_open43_carrot = v43_lead8_open43 + carrot tie-break; section copied from v43_carrot: one change vs V43 -----------------------------------------
